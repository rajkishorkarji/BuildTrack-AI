...
public class DailyLogServiceImpl implements DailyLogService {
    ...
    private TaskEntity resolveTask(DailyLogRequest request, Project project, User user, String role) {
        if (request.progressPercentage() != null && request.taskId() == null) {
            throw new BadRequestException("Select an assigned task before reporting task progress");
        }
        if (request.taskId() == null) return null;
        TaskEntity task = taskRepository.findById(request.taskId()).orElseThrow(() -> new ResourceNotFoundException("Task not found"));
        if (!task.getProject().getId().equals(project.getId()))
            throw new BadRequestException("Selected task does not belong to this project");
        if (!List.of("SUPER_ADMIN", "COMPANY_ADMIN", "PROJECT_MANAGER", "SITE_ENGINEER", "CONTRACTOR").contains(role)
                && (task.getAssignedUser() == null || !task.getAssignedUser().getId().equals(user.getId())))
            throw new BadRequestException("You can report progress only for tasks assigned to you");
        return task;
    }
