...
import { CreditCard, TrendingUp, Download, Building2, BarChart3, Receipt, Search } from 'lucide-react';
...
export default function SuperAdminFinance() {
  ...
  const totalMRR = 0; // SaaS MRR is reported from verified Razorpay subscription payments only.
    ...
    <div className="dashboard-page">
      <section className="hero-row">
        ...
        <button type="button" className="secondary-button" onClick={() => {
          const csv = 'Invoice,Amount,Company,Status,Date\n' + finances.map(f => `${f.invoiceNo},${formatINR(f.amount)},${f.companyName},${f.status},${f.date||'Today'}`).join('\n');
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '14px', marginTop: '20px' }}>
        {[
          { label: 'Platform Billed Revenue', value: formatINR(totalPlatformBilled), color: 'var(--green)' },
          { label: 'Paid Collections', value: formatINR(totalPaid), color: 'var(--blue)' },
          { label: 'SaaS Monthly MRR', value: formatINR(totalMRR), color: 'var(--purple)' },
          { label: 'SaaS Annual ARR', value: formatINR(totalMRR * 12), color: 'var(--orange)' },
          ...
          <tbody>
            {filtered.map(f => (
              <tr key={f.id} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td style={{ padding: '12px', fontWeight: 700, color: 'var(--green)' }}>{formatINR(f.amount)}</td>
