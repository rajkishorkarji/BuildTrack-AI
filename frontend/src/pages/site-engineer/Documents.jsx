...
export default function SEDocuments() {
    ...
    <div className="dashboard-page">
      ...
      <div className="panel" style={{ marginTop: '20px', padding: '16px 20px' }}>
        <div className="search-box" style={{ width: '300px' }}>
          ...
          <input placeholder="Search CAD blueprints, specs..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
          <thead>
            <tr style={{ background: 'var(--panel-soft)', color: 'var(--muted)', borderBottom: '1px solid var(--border)' }}>
              <th style={{ padding: '14px 20px', fontWeight: 600 }}>Blueprint / Document</th>
          ...
          <tbody>
            {(filtered.length > 0 ? filtered : [
              { id: 'se-d1', name: 'Column Concrete Specification C30', category: 'QC Blueprint', status: 'Verified' },
              { id: 'se-d2', name: 'Rebar Placement Schedule Zone B', category: 'CAD Drawing', status: 'Approved' },
            ]).map(d => (
              <tr key={d.id} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td style={{ padding: '14px', color: 'var(--purple)', fontWeight: 600 }}>{d.category || 'Engineering'}</td>
