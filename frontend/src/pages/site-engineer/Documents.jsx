...
export default function Documents() {
  ...
  const upload = async (e) => {
    ...
    setBusy(true);
    try {
      const saved = await documentService.upload(projectId, file);
  ...
  return (
    <div className="dashboard-page">
      ...
      <div
        className="panel"
        style={{ marginTop: 20, padding: 16, display: 'flex', gap: 12, flexWrap: 'wrap' }}
      >
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: 'var(--panel-soft)' }}>
              <th style={{ padding: 14, textAlign: 'left' }}>Document</th>
              <th style={{ padding: 14, textAlign: 'left' }}>Project</th>
              <th style={{ padding: 14, textAlign: 'left' }}>Uploaded By</th>
          ...
          <tbody>
            {filtered.map((d) => (
              <tr key={d.id} style={{ borderTop: '1px solid var(--border)' }}>
                ...
                <td style={{ padding: 14 }}>{d.projectName || '—'}</td>
                <td style={{ padding: 14 }}>{d.uploadedBy || '—'}</td>
                <td style={{ padding: 14, color: 'var(--muted)' }}>
                  {d.createdAt ? new Date(d.createdAt).toLocaleString() : '—'}
                </td>
                <td style={{ padding: 14, textAlign: 'right' }}>
                  <div style={{ display: 'inline-flex', gap: 6 }}>
                    <a className="secondary-button" href={d.fileUrl} target="_blank" rel="noreferrer">
                      <Download size={13} /> Download
                    </a>
                    {canUpload && (
                      <button className="secondary-button" onClick={() => remove(d.id)}>
                        <Trash2 size={13} />
                      </button>
                    )}
                  </div>
