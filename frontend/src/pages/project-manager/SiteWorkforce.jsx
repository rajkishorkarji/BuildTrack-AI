...
import { Users, Search, Plus, ShieldCheck, UserCheck } from 'lucide-react';

export default function PMSiteWorkforce() {
  const { workers = [], addWorker } = useData();
  const [search, setSearch] = useState('');
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState({ fullName: '', role: 'Site Engineer', contractorName: 'Apex Foundations' });
  ...
  const handleAdd = (e) => {
    ...
    setShowAdd(false);
    setForm({ fullName: '', role: 'Site Engineer', contractorName: 'Apex Foundations' });
  };
    ...
    <div className="dashboard-page">
      <section className="hero-row">
        ...
        <button type="button" className="primary-button" onClick={() => setShowAdd(true)}>
          <Plus size={16} /> Deploy Site Personnel
        </button>
      </section>
      ...
      <div className="panel" style={{ marginTop: '20px', padding: '16px 20px' }}>
        <div className="search-box" style={{ width: '300px' }}>
          ...
          <input placeholder="Search site personnel..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
          <thead>
            <tr style={{ background: 'var(--panel-soft)', color: 'var(--muted)', borderBottom: '1px solid var(--border)' }}>
              ...
              <th style={{ padding: '14px', fontWeight: 600 }}>Assigned Contractor / Crew</th>
              <th style={{ padding: '14px 20px', fontWeight: 600 }}>Status</th>
            </tr>
          </thead>
          <tbody>
            {(filtered.length > 0 ? filtered : workers).map(w => (
              <tr key={w.id} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td style={{ padding: '14px', color: 'var(--blue)', fontWeight: 600 }}>{w.role || 'Personnel'}</td>
                <td style={{ padding: '14px', color: 'var(--muted)' }}>{w.contractorName || 'Direct Site Crew'}</td>
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div className="panel" style={{ width: '100%', maxWidth: '440px', padding: '28px', borderRadius: '16px' }}>
            <h2 style={{ fontSize: '20px', marginBottom: '16px' }}>Deploy Site Personnel</h2>
            <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '13px' }}>
              ...
              <div>
                <label style={{ color: 'var(--muted)', display: 'block', marginBottom: '4px' }}>Site Role</label>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '8px' }}>
                ...
                <button type="submit" className="primary-button">Deploy Personnel</button>
              </div>
            </form>
          </div>
        </div>
